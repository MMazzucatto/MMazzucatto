...
$query = "INSERT INTO `tbusuario` (`idusu`, `nomeusu`, `emailusu`, `foneusu`, `tipousu`, `cpfusu`, `cepusu`, `numusu`, `compleusu`) 
VALUES (NULL,'{$nomeusu}','{$emailusu}','{$foneusu}','{$tipousu}','{$cpfusu}','{$cepusu}','{$numusu}','{$compusu}')";
...
function visuUsuarioCodigp($conn,$codigousu){ 
    $query = "select * from tbusuario where codigousu like = {$codigousu}";
    $resultado = mysqli_query($conn, $query);
    return $resultado;
}
